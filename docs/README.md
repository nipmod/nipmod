# Nipmod Docs

Public docs are organized around the product surface that agents and maintainers use today.

## Start

- [Quickstart](quickstart.md)
- [API: search, inspect, install plan](api/search-inspect-install-plan.md)
- [Trust scoring](api/trust-scoring.md)
- [Public API spec](specs/public-api.md)
- [MCP hosts](mcp-hosts.md)
- [Packages](packages.md)

## Architecture

- [Architecture](../ARCHITECTURE.md)
- [API-first 100% plan](api-first-100-plan.md)
- [Package intelligence architecture](package-intelligence-architecture.md)
- [Package intelligence lifecycle](archive/package-intelligence-lifecycle.md)
- [Package intelligence seed v1](archive/seed-v1.md)
- [Trust model](trust-model.md)
- [Source resolver spec](specs/source-resolvers.md)
- [Source crawling spec](specs/source-crawling.md)
- [Trust signals spec](specs/trust-signals.md)
- [Archive records spec](specs/archive-records.md)
- [CLI contract](cli-contract.md)
- [Multi-source registry](multi-source-registry.md)

## Governance

- [GitHub excellence plan](github-excellence.md)
- [Decision records](decisions/README.md)
- [Release process](release-process.md)

## Product Direction

- [Product direction](product/README.md)
- [Product positioning](product/positioning.md)
- [Product decisions](product/decisions.md)
- [Market context](product/market-context.md)
- [Roadmap](product/roadmap.md)

## Operations

- [Adoption readiness](adoption-readiness.md)
- [Audit readiness](audit-readiness.md)
- [Threat model](security/threat-model.md)
- [Data retention](security/data-retention.md)
- [Operator autonomy](operator-autonomy.md)
- [API key schema](api-key-schema.sql)
- [API usage schema](api-usage-schema.sql)
- [API usage metrics schema](api-usage-metrics-schema.sql)
- [Package metadata is untrusted](security/package-metadata-is-untrusted.md)
- [Supply chain security](security/supply-chain.md)
- [Incident publication](incident-publication.md)
- [Independent review](independent-review.md)

## Comparison

- [Category comparison](comparison/README.md)
- [Agent package intelligence benchmark](competitive-benchmark.md)

## Publishing

- [Self-service publishing](self-service-publishing.md)
- [Package claim](package-claim.md)
- [Publish](publish.md)

## Launch Materials

- [API beta launch kit](launch/api-beta.md)
- [API beta post](launch/api-beta-post.md)
- [Launch kit](launch-kit.md)
- [Public launch packet](public-launch-packet.md)
- [Release posting](release-posting.md)
