...
await runCheck("install_plan_canary", () =>
  run(process.execPath, ["--experimental-strip-types", "tools/install-plan-canary.ts", "--base-url", baseUrl], { timeoutMs: 90_000 })
);
await runCheck("archive_depth_canary", () =>
  run(
    process.execPath,
    ["--experimental-strip-types", "tools/archive-depth-canary.ts", "--base-url", baseUrl, "--require-durable"],
    { timeoutMs: 120_000 }
  )
);
