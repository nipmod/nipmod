...
export function registryTrustSummary(index: RegistryIndex): {
  ...
  const hasPackages = index.packages.length > 0;
  ...
  const checks = [
    {
      ...
      ok: !hasPackages || index.packages.every((item) => item.trust.evidence.bundleSignatureVerified),
      text: hasPackages
        ? "Every listed package has a bundle signature."
        : "No public package is listed until a signed bundle passes verification."
    },
    {
      label: "Source tags",
      ok: !hasPackages || index.packages.every((item) => item.trust.evidence.sourceProvenanceVerified),
      text: hasPackages
        ? "Verified packages bind a version tag to a Gitlawb commit."
        : "Future public packages must bind a version tag to a source commit."
    },
    {
      label: "Transparency",
      ok: !hasPackages || index.packages.every((item) => item.trust.evidence.transparencyLogVerified),
      text: hasPackages
        ? "The checkpoint is witnessed outside the registry."
        : "Transparency proof starts with the first verified public package."
    },
    {
      ...
      text: hasPackages
        ? "Each verified package digest has release and security approval receipts."
        : "Release and security approvals are required before a package becomes public."
    },
    {
  ...
  return {
    cards: [
      { label: "Packages", value: String(index.packages.length) },
      { label: "Quorum", value: quorumRequired ? (hasPackages ? `${quorumPassed}/${index.packages.length}` : "pending first package") : "not declared" },
