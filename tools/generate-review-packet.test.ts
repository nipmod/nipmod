...
describe("independent review packet generator", () => {
  test("writes a reproducible external review packet", async () => {
    ...
    try {
      const result = spawnSync(process.execPath, ["tools/generate-review-packet.ts", outPath], {
        cwd: root,
        encoding: "utf8"
      });
      ...
      expect(packet).toContain("node --experimental-strip-types tools/verify-all.ts --prod");
      expect(packet).toContain("node --experimental-strip-types tools/prod-synthetic-monitor.ts");
      expect(packet).toContain("node --experimental-strip-types tools/restore-drill.ts");
      expect(packet).toContain("node --experimental-strip-types tools/public-proof-loop.ts --registry https://nipmod.com/registry/packages.json");
  ...
  test("embeds required evidence outputs when an evidence directory is supplied", async () => {
    ...
    try {
      ...
      const result = spawnSync(process.execPath, ["tools/generate-review-packet.ts", outPath, "--evidence-dir", evidenceDir], {
        cwd: root,
        encoding: "utf8"
      });
      expect(result.status, result.stderr || result.stdout).toBe(0);
