...
Summarize source, license, trust score, decision, warnings and install command.
...
```bash
curl 'https://nipmod.com/api/search?q=markdown%20parser&sources=npm,pypi,github,mcp&limit=5'
curl 'https://nipmod.com/api/inspect?source=npm&name=marked'
curl 'https://nipmod.com/api/install-plan?source=npm&name=marked'
```
