import { hasApiAccessTier } from "../../../../lib/api-auth";
import { adminCorsPolicy, apiOptions, createApiHttpContext } from "../../../../lib/api-http";
import { apiJsonWithUsage } from "../../../../lib/api-response";
import { readAdminSummary } from "../../../../lib/admin-summary";
import { checkApiRateLimitAsync } from "../../../../lib/rate-limit";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";

export function OPTIONS(request: Request): Response {
  return apiOptions(createApiHttpContext(request), { corsPolicy: adminCorsPolicy(request) });
}

export async function GET(request: Request): Promise<Response> {
  const context = createApiHttpContext(request);
  const corsPolicy = adminCorsPolicy(request);
  const rateLimit = await checkApiRateLimitAsync(request, { limit: 30, name: "admin-summary", windowMs: 60_000 }, context, { corsPolicy });
  if (!rateLimit.ok) {
    return rateLimit.response!;
  }

  if (!hasApiAccessTier(rateLimit.access, "admin")) {
    return apiJsonWithUsage(
      request,
      {
        code: "insufficient_api_access",
        error: "admin summary requires an admin API key",
        retryable: false,
        source: null,
        status: 403,
        type: "dev.nipmod.api-error.v1"
      },
      {
        access: rateLimit.access,
        context,
        corsPolicy,
        headers: rateLimit.headers,
        status: 403
      }
    );
  }

  const url = new URL(request.url);
  const limit = readLimit(url.searchParams.get("limit"));
  const since = readSince(url.searchParams.get("hours"));
  const summary = await readAdminSummary({ limit, since });
  return apiJsonWithUsage(request, summary, {
    access: rateLimit.access,
    context,
    corsPolicy,
    headers: rateLimit.headers
  });
}

function readLimit(value: string | null): number {
  if (!value) {
    return 20;
  }
  const parsed = Number.parseInt(value, 10);
  if (!Number.isFinite(parsed) || String(parsed) !== value.trim() || parsed < 1 || parsed > 100) {
    return 20;
  }
  return parsed;
}

function readSince(value: string | null): Date {
  const hours = value ? Number.parseInt(value, 10) : 24;
  const safeHours = Number.isFinite(hours) && String(hours) === value?.trim() && hours >= 1 && hours <= 168 ? hours : 24;
  return new Date(Date.now() - safeHours * 60 * 60_000);
}
