...
- `/`: main product, source coverage, registry preview.
- `/packages`: full package archive.
- `/packages/[packageName]`: package detail pages with trust/proof/install.
- `/setup`: optional local agent onboarding for workspace writes.
- `/mcp`: local MCP plus hosted read-only MCP.
- `/platforms`: current API source and access matrix.
