...
describe("nipmod CLI", () => {
  ...
  test("sets up Claude Code, OpenCode, Cursor and Hermes MCP config", async () => {
    ...
    expect(hermes.data.files).toHaveLength(1);
    expect(hermes.data.files[0]?.path).toBe(hermesConfig);
  ...
  test("signed quarantine advisory blocks audit and ci without mutating the public feed", async () => {
    ...
    const beforePublicFeed = await readFile(fixture.advisoriesPath, "utf8");
    ...
    expect(await readFile(fixture.advisoriesPath, "utf8")).toBe(beforePublicFeed);
  });
...
}): Promise<{ advisoriesPath: string; args: string[]; canonical: string }> {
