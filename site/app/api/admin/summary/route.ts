...
export async function GET(request: Request): Promise<Response> {
  ...
  const rateLimit = await checkApiRateLimitAsync(request, { limit: 30, name: "admin-summary", windowMs: 60_000 }, context, {
  ...
  if (!hasApiAccessTier(rateLimit.access, "admin")) {
    return apiJsonWithUsage(
      request,
      {
        ...
        error: "admin summary requires an admin key or password",
        retryable: false,
        source: null,
        status: 403,
