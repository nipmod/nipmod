...
describe("nipmod MCP server", () => {
  test("initializes and lists accurate default tool safety annotations", async () => {
    ...
    expect(list.result.tools.map((tool: { name: string }) => tool.name)).toEqual([
      ...
      "nipmod.audit",
      "nipmod.deep_scan",
      "nipmod.sandbox_audit",
      "nipmod.sandbox_runtime",
      "nipmod.sbom",
      "nipmod.explain"
    ]);
    ...
    expect(
      ...
      "nipmod.sandbox_audit": false,
      "nipmod.sandbox_runtime": false,
      "nipmod.sbom": false,
  ...
  test("runs cached sandbox audit through tools/call without write capability", async () => {
    const workspace = await mkdtemp(join(tmpdir(), "nipmod-mcp-sandbox-audit-"));
    const cacheDir = join(await mkdtemp(join(tmpdir(), "nipmod-mcp-sandbox-cache-")), "cache");
    await writeFile(join(workspace, "package.json"), JSON.stringify({ name: "mcp-safe" }, null, 2));
    const server = createNipmodMcpServer();
    await initialize(server);
    ...
    expect(second.result.structuredContent.cache.hit).toBe(true);
    expect(second.result.structuredContent.cache.cacheKey).toBe(first.result.structuredContent.cache.cacheKey);
  });

  test("plans sandbox runtime through tools/call without executing in dry-run mode", async () => {
    const workspace = await mkdtemp(join(tmpdir(), "nipmod-mcp-sandbox-runtime-"));
    await mkdir(join(workspace, "src"));
    await writeFile(join(workspace, "package.json"), JSON.stringify({ name: "mcp-runtime-safe" }, null, 2));
