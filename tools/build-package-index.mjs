...
const PUBLIC_REGISTRY_EXCLUSIONS_PATH = join(ROOT, "tools", "public-registry-exclusions.json");
...
function registryExcludedPackageKeys(
  value = process.env.NIPMOD_REGISTRY_EXCLUDE_PACKAGES,
  exclusionsPath = process.env.NIPMOD_REGISTRY_EXCLUSIONS_PATH ?? PUBLIC_REGISTRY_EXCLUSIONS_PATH
) {
  const excluded = loadRegistryExclusionFile(exclusionsPath);
  if (value instanceof Set) {
    for (const item of value) {
      addRegistryExclusion(excluded, item);
    }
    return excluded;
  }
...
function loadRegistryExclusionFile(path) {
  ...
  try {
    ...
    for (const entry of packages) {
      addRegistryExclusion(excluded, entry);
    }
  } catch (error) {
    if (!isNotFound(error)) {
      throw new Error(`registry exclusion file is invalid: ${error instanceof Error ? error.message : "unknown error"}`);
    }
  }
  return excluded;
}

function addRegistryExclusion(excluded, entry) {
  ...
  for (const key of ["key", "canonical", "package"]) {
    if (typeof entry[key] === "string" && entry[key].trim()) {
      excluded.add(entry[key].trim());
    }
  }
  if (typeof entry.canonical === "string" && typeof entry.version === "string") {
    excluded.add(`${entry.canonical}@${entry.version}`);
  }
}
