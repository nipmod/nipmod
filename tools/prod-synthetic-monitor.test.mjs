...
describe("production synthetic monitor", () => {
  ...
  test("passes the public production contract for an intentionally empty archive", async () => {
    const fixture = createFixture({ emptyArchive: true });
    const result = await runSyntheticMonitor({
      endpoints: fixture.endpoints,
      expected: fixture.expected,
      fetchFn: fixture.fetchFn,
...
function createFixture({
  ...
  const endpoints = {
    ...
    archiveConfirm: "https://nipmod.test/api/archive/confirm",
    archivePrepare: "https://nipmod.test/api/archive/prepare",
    archiveSearch: "https://nipmod.test/api/archive/search",
  ...
  if (emptyArchive) {
    discovery.transparency = {
      logId: expected.logId,
      status: "pending_first_public_package"
    };
  }
  const routes = {
    ...
    [`GET ${endpoints.archiveStatus}`]: jsonResponse({
      configured: false,
      driver: "supabase-rest",
      missing: [
        "NIPMOD_ARCHIVE_SUPABASE_URL",
        "NIPMOD_ARCHIVE_WRITE_TOKEN",
        "NIPMOD_ARCHIVE_SUPABASE_SERVICE_ROLE_KEY or NIPMOD_ARCHIVE_SUPABASE_PUBLISHABLE_KEY"
      ],
    ...
    [`POST ${endpoints.archiveConfirm}`]: jsonResponse({
      dryRun: true,
      record: {
        archive: {
    ...
    [`GET ${endpoints.registry}`]: jsonResponse(
      emptyArchive
        ...
        : registryFixture(checkpoint, registryPackagePatch)
    ),
    ...
    [`GET ${endpoints.quorumReceipts}`]: jsonResponse({
      receipts: emptyArchive ? [] : [{ package: "pkg:did:key:z6Mkpkg/example", version: "0.1.0" }],
    ...
    [`GET ${endpoints.witnessHealth}`]: jsonResponse({
      ok: true,
      lastError: emptyArchive ? "fetch failed 404 at https://nipmod.test/transparency/witness-request.json" : null,
      lastRunAt: "2026-05-16T12:41:00.000Z",
      lastWitness: emptyArchive
        ? null
        : {
