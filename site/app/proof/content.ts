...
const proofPackageName = "<package-name>";
const proofPackage = registry.packages.find((pkg) => pkg.name === proofPackageName);
const proofSubject = proofPackage ? `${proofPackage.canonical}@${proofPackage.version}` : "<package-specifier>";

export const proofContent = {
  headline: "Proof you can run",
  lead: "Verify the package gates before a public package enters the archive.",
  packageName: proofPackageName,
  registry: {
    count: registry.packages.length,
    rootHash: treeHead?.rootHash ?? "missing",
    treeSize: treeHead?.treeSize ?? 0,
    trust: registry.packages.length === 0 ? "awaiting first package" : verified.length === registry.packages.length ? "verified/100" : "review required"
  },
  safeCommands: [
