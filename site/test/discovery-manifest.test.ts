...
describe("nipmod discovery manifest", () => {
  ...
  test("keeps nested objects exact and discovery only", () => {
    ...
    expect(Object.keys(manifest.bankr.proof).sort()).toEqual(["agentWorkflow", "expectedSteps", "package", "runtimeSmoke"]);
  ...
  test("publishes a complete agent runbook from the machine manifest", () => {
    expect(manifest.docs).toEqual({
      ...
      hermes: "https://nipmod.com/setup",
      hermesBundle: "https://nipmod.com/integrations/hermes/skill-bundles/nipmod.yaml",
      hermesSkill: "https://nipmod.com/integrations/hermes/nipmod/SKILL.md",
    ...
    expect(manifest.agent.commands).toEqual({
      ...
      setupCursorMcp: "nipmod setup cursor",
      setupCursorOneClick:
