...
import { bindSandboxAuditReportToDecision, formatSandboxDecisionBinding, readSandboxDecisionFile } from "./sandbox-decision-binding.js";
import { formatSandboxAuditReport, runSandboxAudit, type SandboxAuditReport } from "./sandbox-audit.js";
import { formatSandboxRuntimeReport, runSandboxRuntime, type SandboxRuntimeNetworkPolicy, type SandboxRuntimeReport, type SandboxRuntimeRuntime } from "./sandbox-runtime.js";
...
async function runCommand(command: string | undefined, args: string[]): Promise<CliResult> {
  switch (command) {
    ...
    case "sandbox-audit":
      return sandboxAuditCommand(args);
    case "sandbox-runtime":
      return sandboxRuntimeCommand(args);
    case "ci":
...
async function sandboxAuditCommand(args: string[]): Promise<CliResult> {
  const target = optionalFirstPositional(args) ?? optionalFlagValue(args, "--dir") ?? process.cwd();
  const cacheDir = optionalFlagValue(args, "--cache-dir") ?? undefined;
  const decisionPath = optionalFlagValue(args, "--decision") ?? undefined;
  const maxFiles = optionalPositiveInteger(args, "--max-files");
  const maxBytesPerFile = optionalPositiveInteger(args, "--max-bytes-per-file");
  const maxArtifactBytes = optionalPositiveInteger(args, "--max-artifact-bytes");
  const maxArtifactEntries = optionalPositiveInteger(args, "--max-artifact-entries");
  const report: SandboxAuditReport = await runSandboxAudit({
    ...(cacheDir ? { cacheDir } : {}),
    ...(maxArtifactBytes !== undefined ? { maxArtifactBytes } : {}),
    ...(maxArtifactEntries !== undefined ? { maxArtifactEntries } : {}),
    ...(maxBytesPerFile !== undefined ? { maxBytesPerFile } : {}),
    ...(maxFiles !== undefined ? { maxFiles } : {}),
    force: hasFlag(args, "--force"),
    path: target
  });
  const decisionBinding = decisionPath
    ? bindSandboxAuditReportToDecision({
        decision: await readSandboxDecisionFile(decisionPath),
        report,
        targetConfirmed: hasFlag(args, "--target-confirmed")
      })
    : null;
  const ok = report.verdict !== "blocked" && (!decisionBinding || decisionBinding.ok);
  return {
    ok,
    data: {
      message: decisionBinding
        ? `${formatSandboxAuditReport(report)}\n${formatSandboxDecisionBinding(decisionBinding)}`
        : formatSandboxAuditReport(report),
      report,
      ...(decisionBinding ? { decisionBinding } : {})
    },
    exitCode: report.verdict === "blocked" ? 7 : decisionBinding && !decisionBinding.ok ? 12 : 0
  };
}

async function sandboxRuntimeCommand(args: string[]): Promise<CliResult> {
  const delimiter = args.indexOf("--");
  if (delimiter === -1) {
    throw new Error("usage: nipmod sandbox-runtime <path> [--confirm-runtime] -- <command> [args...]");
  }
  const beforeCommand = args.slice(0, delimiter);
  const command = args.slice(delimiter + 1);
  if (command.length === 0) {
    throw new Error("sandbox-runtime requires a command after --");
  }
  const target = optionalFirstPositional(beforeCommand) ?? optionalFlagValue(beforeCommand, "--dir") ?? process.cwd();
  const cacheDir = optionalFlagValue(beforeCommand, "--cache-dir") ?? undefined;
  const decisionPath = optionalFlagValue(beforeCommand, "--decision") ?? undefined;
  const maxFiles = optionalPositiveInteger(beforeCommand, "--max-files");
  const maxBytesPerFile = optionalPositiveInteger(beforeCommand, "--max-bytes-per-file");
  const maxUploadBytes = optionalPositiveInteger(beforeCommand, "--max-upload-bytes");
  const maxArtifactBytes = optionalPositiveInteger(beforeCommand, "--max-artifact-bytes");
  const maxArtifactEntries = optionalPositiveInteger(beforeCommand, "--max-artifact-entries");
  const timeoutMs = optionalPositiveInteger(beforeCommand, "--timeout-ms");
  const runtime = optionalSandboxRuntime(beforeCommand);
  const networkPolicy = optionalSandboxNetworkPolicy(beforeCommand);
  const report: SandboxRuntimeReport = await runSandboxRuntime({
    ...(cacheDir ? { cacheDir } : {}),
    ...(maxArtifactBytes !== undefined ? { maxArtifactBytes } : {}),
    ...(maxArtifactEntries !== undefined ? { maxArtifactEntries } : {}),
    ...(maxBytesPerFile !== undefined ? { maxBytesPerFile } : {}),
    ...
    allowBlocked: hasFlag(beforeCommand, "--allow-blocked"),
    allowReview: hasFlag(beforeCommand, "--allow-review"),
    command,
    confirmRuntime: hasFlag(beforeCommand, "--confirm-runtime"),
    ...(decisionPath ? { decision: await readSandboxDecisionFile(decisionPath) } : {}),
    dryRun: hasFlag(beforeCommand, "--dry-run"),
    force: hasFlag(beforeCommand, "--force-audit") || hasFlag(beforeCommand, "--force"),
    path: target,
    targetConfirmed: hasFlag(beforeCommand, "--target-confirmed")
  });
  const ok = report.verdict === "pass" || report.verdict === "skipped";
  return {
    ok,
    data: {
      message: formatSandboxRuntimeReport(report),
      report
    },
    exitCode: sandboxRuntimeExitCode(report)
  };
}
...
function sandboxRuntimeExitCode(report: SandboxRuntimeReport): number {
  if (report.verdict === "pass" || report.verdict === "skipped") {
    return 0;
  }
  if (report.decisionBinding && !report.decisionBinding.ok) {
    return 12;
  }
  return report.verdict === "review" ? 12 : 7;
}
...
function optionalSandboxRuntime(args: readonly string[]): SandboxRuntimeRuntime | undefined {
  const value = optionalFlagValue(args, "--runtime");
  if (value === null) return undefined;
  if (value === "node22" || value === "node24" || value === "node26" || value === "python3.13") {
    return value;
  }
  throw new Error("--runtime must be one of node22, node24, node26, python3.13");
}

function optionalSandboxNetworkPolicy(args: readonly string[]): SandboxRuntimeNetworkPolicy | undefined {
  const value = optionalFlagValue(args, "--network");
  if (value === null) return undefined;
  if (value === "allow-all" || value === "deny-all") {
    return value;
  }
  throw new Error("--network must be one of deny-all, allow-all");
}
