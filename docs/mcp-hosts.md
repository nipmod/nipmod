...
Nipmod also exposes a hosted read-only MCP endpoint at `https://nipmod.com/api/mcp`. Use it when an agent needs verified archive discovery, external package resolution, exact metadata, trust inspection, install planning or a demo flow. It never reads local files and never writes a workspace.
...
- Read-only tools: `search`, `resolve`, `view`, `inspect`, `install_plan`, `external_install_plan`, `update_plan`, `demo`, `claim_verify`, `claim_index`, `verify`, `audit`, `sbom` and `explain`.
