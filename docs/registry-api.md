...
The verified registry and the external resolver are separate surfaces.

Verified Nipmod packages live under `/registry/packages.json` and require Nipmod verification gates. External packages from npm, PyPI, GitHub, Hugging Face and MCP are resolved through `/api/search`, `/api/inspect` and `/api/install-plan` as `external_indexed` records. Original package owners keep ownership; Nipmod adds source context, trust signals and install plans.

## Package intelligence archive boundary

The package intelligence archive is the durable layer above the external resolver. It is not a package rehost and it is not a mirror.

| Endpoint | Purpose |
| --- | --- |
| `/api/archive/prepare` | Convert an external resolver record into a deterministic package intelligence record. |
| `/api/archive/search` | Search persisted package intelligence records when the production store is configured. |
| `/api/archive/confirm` | Confirm useful package usage and persist the record through an authenticated write token. |
| `/api/archive/status` | Report whether the durable archive store is enabled. |

Persisted package intelligence statuses are `external_indexed`, `agent_confirmed`, `claimed`, `verified_nipmod`, `quarantined` and `yanked`. Public callers can prepare records and dry run confirmations, but durable writes require server-side store credentials and the archive write token.
