...
function installCommandRisk(commands: string[]): "low" | "medium" | "high" {
  if (commands.some((command) => /curl\s+[^|]+\|\s*(?:sh|bash)|wget\s+[^|]+\|\s*(?:sh|bash)/i.test(command))) {
    return "high";
  }
  if (commands.some((command) => /(?:^|\s)(?:sudo|chmod|chown|rm\s+-rf)\b/i.test(command))) {
    return "high";
  }
  if (commands.some((command) => /(?:&&|\|\||;|`|\$\()/i.test(command))) {
    return "medium";
  }
  return "low";
}
