...
describe("restore drill", () => {
  ...
  test("passes cleanly when the public archive is intentionally empty", async () => {
...
function createEmptyFixture() {
  ...
  return {
    ...
    fetchFn: fakeFetch(routes),
    gitLsRemoteFn: async () => {
      throw new Error("git restore must be skipped for an empty archive");
    }
  };
}
