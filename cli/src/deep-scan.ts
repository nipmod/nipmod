...
export interface DeepScanReport {
  ...
  riskInventory: DeepScanRiskInventory;
  summary: {
...
export const DEFAULT_DEEP_SCAN_LIMITS = {
  maxArtifactBytes: 25 * 1024 * 1024,
  maxArtifactEntries: 200,
  maxBytesPerFile: 256 * 1024,
  maxFiles: 750
} as const;
const DEFAULT_MAX_FILES = DEFAULT_DEEP_SCAN_LIMITS.maxFiles;
const DEFAULT_MAX_BYTES_PER_FILE = DEFAULT_DEEP_SCAN_LIMITS.maxBytesPerFile;
const DEFAULT_MAX_ARTIFACT_BYTES = DEFAULT_DEEP_SCAN_LIMITS.maxArtifactBytes;
const DEFAULT_MAX_ARTIFACT_ENTRIES = DEFAULT_DEEP_SCAN_LIMITS.maxArtifactEntries;
...
const INSTALL_HOOK_CATEGORIES = new Set([
  "cargo-build-script",
  "composer-lifecycle-script",
  "npm-lifecycle-script",
  "python-build-backend-risk",
  "ruby-native-extension-hook"
]);
const NETWORK_CATEGORIES = new Set(["direct-dependency-url", "downloaded-file-execution", "network-fetch", "network-listener", "remote-shell"]);
const FILESYSTEM_WRITE_CATEGORIES = new Set(["destructive-command", "filesystem-write", "persistence-hook"]);
const PROCESS_EXECUTION_CATEGORIES = new Set(["downloaded-file-execution", "encoded-execution", "obfuscated-execution", "process-execution", "remote-shell"]);
const CREDENTIAL_CATEGORIES = new Set(["credential-access", "secret-exfiltration"]);
const ARTIFACT_GAP_CATEGORIES = new Set(["artifact-scan-limit", "nested-artifact-depth"]);
...
const TEXT_RULES: TextRule[] = [
  ...
  },
  {
    ...
    regex: /\b(fs\.(writeFile|appendFile|rm|rmdir|unlink|rename)|writeFileSync|appendFileSync|Deno\.write(File|TextFile)?|Bun\.write|file_put_contents|File\.write|FileUtils\.rm_rf|Path\([^)]*\)\.write_text|shutil\.(rmtree|move)|std::fs::(write|remove_file|remove_dir_all)|open\s*\([^)]{0,120}["'][wa]\b)\b/i,
    severity: "medium",
    recommendation: "Review filesystem write behavior in an isolated sandbox before allowing workspace access."
  },
  {
    category: "persistence-hook",
    regex: /\b(crontab|launchctl|LaunchAgents|systemctl\s+enable|schtasks|StartupItems|HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run)\b|(?:^|[\s"'`>])(?:\/etc\/profile|\.bashrc|\.zshrc)\b/i,
    severity: "high",
    recommendation: "Block package code that attempts persistence until reviewed in an isolated runtime sandbox."
  },
  {
    category: "network-listener",
    regex: /\b(http\.createServer|https\.createServer|net\.createServer|socket\.bind|socketserver|app\.listen|server\.listen|Deno\.serve|Bun\.serve)\b/i,
    severity: "medium",
    recommendation: "Review network listener behavior before allowing runtime execution."
  },
  {
...
export async function deepScanProject(options: DeepScanOptions): Promise<DeepScanReport> {
  ...
  const riskInventory = buildRiskInventory(state);
...
async function scanFile(absolutePath: string, state: ScanState): Promise<void> {
  ...
  if (basename(rel) === "composer.json") {
    scanComposerJson(rel, content, state);
  }
  if (basename(rel) === "Cargo.toml") {
    scanCargoToml(rel, content, state);
  }
  if (basename(rel) === "build.rs") {
    scanCargoBuildScript(rel, content, state);
  }
  if (basename(rel) === "extconf.rb" || basename(rel) === "Rakefile" || rel.endsWith(".gemspec")) {
    scanRubyBuildFile(rel, content, state);
  }
...
function scanVirtualArtifactEntry(
  ...
  if (basename(safePath) === "composer.json") {
    scanComposerJson(virtualPath, text, state);
  }
  if (basename(safePath) === "Cargo.toml") {
    scanCargoToml(virtualPath, text, state);
  }
  if (basename(safePath) === "build.rs") {
    scanCargoBuildScript(virtualPath, text, state);
  }
  if (basename(safePath) === "extconf.rb" || basename(safePath) === "Rakefile" || safePath.endsWith(".gemspec")) {
    scanRubyBuildFile(virtualPath, text, state);
  }
...
function scanComposerJson(file: string, content: string, state: ScanState): void {
  let parsed: unknown;
  try {
    parsed = JSON.parse(content);
  } catch {
    addFinding(state, {
      category: "manifest-parse",
      evidence: "composer.json is not valid JSON",
      file,
      recommendation: "Fix manifest parsing before an agent trusts install metadata.",
      severity: "medium"
    });
  ...
  for (const [name, value] of Object.entries(scripts)) {
    ...
    const scriptText = Array.isArray(value) ? value.filter((item): item is string => typeof item === "string").join(" && ") : value;
    if (!scriptText) {
      continue;
    }
    const severity = /post-install|pre-install|post-update|pre-update/i.test(name) || isHighRiskShell(scriptText) ? "high" : "medium";
    addFinding(state, {
      category: "composer-lifecycle-script",
      evidence: `scripts.${name} = ${compact(scriptText)}`,
      file,
      recommendation: "Review Composer lifecycle scripts before allowing package install or update execution.",
      severity
    });
  }
}

function scanCargoToml(file: string, content: string, state: ScanState): void {
  if (/\bbuild\s*=\s*["'][^"']+["']/i.test(content)) {
    addFinding(state, {
      category: "cargo-build-script",
      evidence: "Cargo.toml declares a custom build script.",
      file,
      recommendation: "Review Rust build scripts because cargo can execute build.rs during builds.",
      severity: "medium"
    });
  }
}

function scanCargoBuildScript(file: string, content: string, state: ScanState): void {
  addFinding(state, {
    category: "cargo-build-script",
    evidence: "build.rs is present.",
    file,
    recommendation: "Review Rust build script behavior before allowing cargo build or install.",
    severity: isHighRiskShell(content) ? "high" : "medium"
  });
}

function scanRubyBuildFile(file: string, content: string, state: ScanState): void {
  if (/create_makefile|mkmf|spec\.extensions|Gem::Specification|Rake::Task/i.test(content)) {
    addFinding(state, {
      category: "ruby-native-extension-hook",
      evidence: `${basename(file)} can participate in gem build or native extension setup.`,
      file,
      recommendation: "Review Ruby gem build hooks before allowing bundle install or gem install.",
      severity: isHighRiskShell(content) ? "high" : "medium"
    });
  }
}
...
function buildChecks(state: ScanState, counts: { highCount: number; mediumCount: number }): DeepScanCheck[] {
  ...
  return [
    ...
    {
      evidence:
        riskInventory.installHookCount > 0
          ? `${riskInventory.installHookCount} package install or build hook finding(s) found.`
          : "No package lifecycle, build backend or native extension hooks found.",
      id: "package-install-hooks",
      label: "Package install hooks",
      status: riskInventory.installHookCount > 0 ? "warning" : "pass"
    },
    {
      evidence:
        riskInventory.filesystemWriteCount > 0
          ? `${riskInventory.filesystemWriteCount} filesystem write or persistence finding(s) found.`
          : "No filesystem write or persistence patterns found.",
      id: "filesystem-boundary",
      label: "Filesystem write boundary",
      status: riskInventory.filesystemWriteCount > 0 ? "warning" : "pass"
    },
    {
      evidence:
        riskInventory.networkAccessCount > 0
          ? `${riskInventory.networkAccessCount} network fetch, listener or remote artifact finding(s) found.`
          : "No network fetch or listener patterns found.",
      id: "network-boundary",
      label: "Network boundary",
      status: riskInventory.networkAccessCount > 0 ? "warning" : "pass"
    },
    {
...
function buildRiskInventory(state: ScanState): DeepScanRiskInventory {
  const categories = new Set(state.findings.map((finding) => finding.category));
  const countWhere = (items: Set<string>) => state.findings.filter((finding) => items.has(finding.category)).length;
  return {
    artifactScanGapCount: countWhere(ARTIFACT_GAP_CATEGORIES),
    credentialAccessCount: countWhere(CREDENTIAL_CATEGORIES),
    filesystemWriteCount: countWhere(FILESYSTEM_WRITE_CATEGORIES),
    installHookCount: countWhere(INSTALL_HOOK_CATEGORIES),
    networkAccessCount: countWhere(NETWORK_CATEGORIES),
    packageHookFiles: uniqueStrings(
      state.findings.filter((finding) => INSTALL_HOOK_CATEGORIES.has(finding.category)).map((finding) => finding.file)
    ).slice(0, 20),
    processExecutionCount: countWhere(PROCESS_EXECUTION_CATEGORIES),
    promptInjectionCount: state.findings.filter((finding) => finding.category === "metadata-prompt-injection").length,
    riskCategories: [...categories].sort(),
    sandboxBoundary: {
      filesystem: "read-only-static-analysis",
      network: "disabled",
      runtime: "no-code-execution",
