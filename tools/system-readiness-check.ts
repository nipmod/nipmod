...
async function checkStaticSystemReceipt() {
  ...
  assertEqual("system_receipt_package_count", receipt.sharedArchive.packageCount, 0);
  ...
  assertText("system_receipt_scope", receipt.meaning, "public package archive");
  assertText("system_receipt_remote_mcp", receipt.meaning, "hosted read-only MCP access");
  assertText("system_receipt_quorum", receipt.meaning, "verification gates");
...
async function checkArchiveInvariants() {
  ...
  if (hasVerifiedArchivePackages()) {
    assertEqual("archive_quorum_policy", registry.quorumPolicy?.id, "nipmod-quorum-release-v1");
  } else {
    assertEqual("archive_empty_quorum_policy", registry.quorumPolicy, undefined);
  }
  ...
  if (!hasVerifiedArchivePackages()) {
    assertEqual("archive_empty_public_packages", registry.packages.length, 0);
    assertEqual("archive_empty_proof_package", state.receipt.proofPackage, null);
    pass("archive_package_documents", { mode: "empty-public-archive" });
    return;
  }
...
async function checkDiscoveryBinding() {
  ...
  assertText("llms_api_first", llms, "API first package discovery");
  assertText("llms_local_mcp", llms, "nipmod mcp serve");
}
...
async function checkCliSurface() {
  ...
  const search = await run(nodeBin, [cliPath, "search", proofPackageName, "--registry", localRegistryUrl, "--json"]);
  if (!hasVerifiedArchivePackages()) {
    assertText("cli_search_empty_archive", search.stdout, '"total":0');
    assertText("cli_search_empty_message", search.stdout, "No packages found");
  } else {
    assertText("cli_search_archive", search.stdout, proofPackageName);
...
async function checkMcpSurface() {
    ...
    const calls = await mcpRequest([
      {
        ...
        jsonrpc: "2.0",
        method: "tools/call",
        params: { arguments: { query: proofPackageName }, name: "nipmod.search" }
      },
      {
        id: 4,
        jsonrpc: "2.0",
      ...
      },
      {
        ...
        jsonrpc: "2.0",
        method: "tools/call",
        params: { arguments: { host: "Codex", package: proofPackageName }, name: "nipmod.demo" }
      }
    ]);
    assertText("mcp_search_empty_archive", JSON.stringify(calls[0]), '"total":0');
    assertText("mcp_inspect_empty_archive", JSON.stringify(calls[1]), "package is missing from the registry");
    assertText("mcp_demo_empty_archive", JSON.stringify(calls[2]), proofPackageName);
    return;
  }
  ...
  assertText("mcp_search_archive", JSON.stringify(calls[0]), proofPackageName);
  ...
  assertText("mcp_demo_archive", JSON.stringify(calls[3]), proofPackageName);
}
...
async function checkRemoteMcpSurface() {
  ...
  if (!hasVerifiedArchivePackages()) {
    assertText("remote_mcp_search_empty_archive", JSON.stringify(search), '"total":0');
  } else {
    assertText("remote_mcp_search_archive", JSON.stringify(search), proofPackageName);
  ...
  const resolve = await remoteMcpRequest({
    id: 34,
    jsonrpc: "2.0",
    method: "tools/call",
    params: { arguments: { limit: 1, query: "telegram bot", sources: ["npm"] }, name: "nipmod.resolve" }
  });
  assertText("remote_mcp_resolve_external", JSON.stringify(resolve), "external_indexed");
  assertText("remote_mcp_resolve_ownership", JSON.stringify(resolve), "original creators");
...
async function checkWriteBoundaries() {
  ...
  try {
    ...
    const plan = await run(nodeBin, [cliPath, "install", "--plan", proofPackage, "--dir", workspace, "--json"], { allowFailure: true });
    assertEqual("install_plan_no_workspace_write_exit", plan.code, hasVerifiedArchivePackages() ? 0 : 1);
    if (!hasVerifiedArchivePackages()) {
      assertText("install_plan_empty_archive_error", plan.stdout, "no trusted package matches");
    }
...
async function checkLiveSystemEndpoints() {
  ...
  const endpoints = [
    ["live_setup", state.receipt.entrypoints.humanSetup, ["Pick your agent", "curl https://nipmod.com/i|bash"]],
...
async function checkParallelArchiveAccess() {
  const tasks = [
    async () => assertText("parallel_registry", await fetchText(state.receipt.sharedArchive.registry), hasVerifiedArchivePackages() ? proofPackage.replace("@0.1.0", "") : '"packages": []'),
    async () =>
      assertText(
        "parallel_cli_search",
        (await run(nodeBin, [cliPath, "search", proofPackageName, "--online", "--json"])).stdout,
        hasVerifiedArchivePackages() ? "verified/100" : '"total":0'
      ),
    async () =>
      assertText(
        "parallel_mcp_search",
        ...
        hasVerifiedArchivePackages() ? proofPackageName : '"total":0'
      ),
    async () =>
      assertText(
